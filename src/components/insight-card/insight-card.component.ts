...
export class InsightCardComponent {
  ...
  getTitle(): string {
    // Use the title from the insight if available, otherwise generate from category
    return this.insight().title || this.getDefaultTitle();
  }
  ...
  getMaxValue(): number {
    ...
    const values = [context.currentValue, context.targetValue];
    if (context.projectedValue !== null) {
      values.push(context.projectedValue);
    }
